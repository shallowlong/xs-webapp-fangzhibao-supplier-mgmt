...
const verifyTokenMidware = async (req, res, next) => {
	try {
		...
		if (!token) {
			return res
				.status(401)
				.json({ success: false, message: "认证令牌格式错误" });
		}
		...
		if (!isValid) {
			return res
				.status(401)
				.json({ success: false, message: "无效的认证令牌" });
		}
...
router.post("/getToken", async (req, res) => {
	try {
		...
		if (!username || !password) {
			return res
				.status(400)
				.json({ success: false, message: "用户名和密码是必填项" });
		}

		const token = await apiTokenService.validateAndRetrieve(
			username,
			password,
		);
		if (!token) {
			return res
				.status(401)
				.json({ success: false, message: "用户名密码验证失败" });
		}
...
router.get("/getSuppliersByName", verifyTokenMidware, async (req, res) => {
	try {
		...
		if (!name) {
			return res
				.status(400)
				.json({ success: false, message: "供应商名称是必填项" });
		}
...
router.get("/getSuppliersByAddress", verifyTokenMidware, async (req, res) => {
	try {
		...
		if (!address) {
			return res
				.status(400)
				.json({ success: false, message: "地址是必填项" });
		}
